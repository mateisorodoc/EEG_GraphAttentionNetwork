...
The DEAP dataset~\cite{koelstra2012} is the most widely used benchmark for EEG emotion recognition, featuring 32 subjects watching 40 one-minute music videos while 32-channel EEG is recorded. Self-reported arousal and valence ratings (1--9 scale) serve as ground truth. Published results vary widely: the original paper reports 57.6\% accuracy for subject-dependent classification, while subsequent deep learning studies report 90--99\%~\cite{craik2019}. Zhong et al.~\cite{zhong2020} applied regularized Graph Neural Networks under LOSO evaluation, achieving 56.3\% accuracy, consistent with our Tier~3 findings. Zheng and Lu~\cite{zheng2015} reported 69.7\% for valence under subject-dependent evaluation using deep belief networks, aligning with our Tier~1 binary valence result (77.7\%). This consistency across studies validates our tier classification scheme.

The discrepancy between LOSO ($\sim$56\%) and random-split ($>$90\%) results represents a critical confound that our framework aims to quantify.

\subsection{Evaluation Methodology}

Data leakage in neuroimaging classification is well-documented. Varoquaux et al.~\cite{varoquaux2017} demonstrated that improper cross-validation inflates brain decoding accuracy. Roberts et al.~\cite{roberts2017} showed that temporal and hierarchical data structures require specialized CV strategies. Our four-tier framework instantiates these principles specifically for EEG-BCI, extending prior work by separately quantifying trial leakage versus cross-subject generalization difficulty.
...
\end{equation}
where $W$ is the window length (2\,s) and $S$ is the step size. With $S = W/2$ (our configuration), overlap is 50\%. A random split places adjacent windows in different sets with probability 0.32, enabling exploitation of shared signal content.
...
\textbf{Tier~3 (Cross-Subject LOSO):} Leave-One-Subject-Out. Trains on 19 subjects, tests on the held-out subject. Evaluates cross-individual generalization.
...
The DEAP subjects (s01--s20) were selected sequentially. Labels derive from self-report arousal and valence ratings using per-subject median splits, forming four quadrants: HAHV, HALV, LAHV, LALV. The OpenBCI dataset uses experimenter-assigned labels based on emotional content of music stimuli selected from validated databases~\cite{zentner2008}. Both approaches map to the same arousal-valence space, with chance level at 25\% (4-class) or 50\% (binary).
...
Table~\ref{tab:models} presents macro F1 across all tiers. The dominant finding is a 29.5-point F1 drop from T0 to T2, with an additional 10.1-point drop to T3.
...
The same degradation pattern persists: T1$\to$T2 drops of 17.1 (arousal) and 15.6 (valence) points for MLP confirm trial leakage inflates binary results similarly to 4-class. Valence is consistently more discriminable than arousal across all tiers, consistent with prior findings that spectral asymmetry features encode valence information more reliably~\cite{zheng2015}. Notably, arousal T3 (0.516) is barely above chance (0.50), while valence T2 (0.649) remains substantially above chance, suggesting that valence may be more amenable to EEG-based classification than arousal.
...
Attention weight analysis of the trained GAT reveals the top-5 most attended channels: CP2, T7, AF4, P3, CP1. The prominence of temporal (T7) and parietal (CP1, CP2, P3) electrodes is consistent with established neuroscience findings linking these regions to emotional processing~\cite{allen2004}. Frontal attention asymmetry (AF4) aligns with the valence lateralization hypothesis, which posits that the right prefrontal cortex is more involved in withdrawal-related (negative valence) emotions. The learned attention patterns provide interpretable evidence that the GAT captures physiologically meaningful spatial relationships rather than arbitrary feature correlations.
...
Table~\ref{tab:lit} contextualizes our results against published DEAP benchmarks. The alignment between our tier-assigned results and reported values from independent studies provides external validation of the framework.
...
\midrule
Koelstra~\cite{koelstra2012} & Subj.-dep. & 57.6\% Acc & $\sim$T2 \\
Zhong~\cite{zhong2020} & LOSO & 56.3\% Acc & T3 \\
Zheng \& Lu~\cite{zheng2015} & Subj.-dep. & 69.7\% (Val) & $\sim$T1 \\
High-accuracy studies & Random split & 90--99\% & T0 \\
\midrule
...
Our Tier~2 binary valence (0.649 F1) is consistent with Koelstra et al.'s subject-dependent result, and our Tier~3 4-class performance aligns with Zhong et al.'s LOSO finding when accounting for the increased difficulty of 4-class versus binary classification ($\text{chance}_{4\text{-class}} = 0.25$ vs $\text{chance}_{\text{binary}} = 0.50$). The convergence of results from independent implementations under comparable evaluation conditions strengthens confidence in the reliability of T2/T3 as realistic performance benchmarks.
...
\textbf{Domain adaptation:} The T2$\to$T3 drop ($\Delta=-0.101$) suggests that subject-alignment techniques (Euclidean Alignment, CORAL and adversarial domain adaptation) could recover a significant portion of performance lost to inter-subject variability.
...
\bibitem{zentner2008}
M.~Zentner, D.~Grandjean, and K.~R.~Scherer, ``Emotions evoked by the sound of music: Characterization, classification, and measurement,'' \emph{Emotion}, vol.~8, no.~4, pp.~494--521, 2008.
