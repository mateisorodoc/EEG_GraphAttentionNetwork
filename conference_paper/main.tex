...
The DEAP subjects (s01--s20) were selected sequentially as a 20-subject subset due to computational constraints; the full DEAP corpus contains 32 subjects. Labels derive from self-report arousal and valence ratings using per-subject median splits, forming four quadrants: HAHV, HALV, LAHV, LALV. The OpenBCI dataset uses experimenter-assigned labels based on emotional content of music stimuli selected from validated databases~\cite{zentner2008}; the single participant was the lead author, who provided informed self-consent. Both approaches map to the same arousal-valence space, with chance level at 25\% (4-class) or 50\% (binary).
